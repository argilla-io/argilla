...
![A sketch of the Rubrix data model](../images/rubrix_data_model.svg)


### Dataset

A dataset is a collection of [records](#record) of a common type.
You can programmatically [build datasets](../guides/datasets.ipynb) with the Rubrix client and [`log`](#rb-log) them to the web app.
...
These methods allow you to programmatically work with Rubrix datasets.
To find more information about these methods, please check out the {ref}`python_client`.

### rb.init

Initialize the python client: {meth}`rubrix.init`

### rb.log

Log a dataset from the client to the Rubrix web app: {meth}`rubrix.log`

### rb.load

Load a Rubrix dataset from the web app into the client: {meth}`rubrix.load`
