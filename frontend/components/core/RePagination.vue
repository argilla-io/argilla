<template>
  <div class="pagination__container">
    <div class="pagination__selector" v-click-outside="closePageSizeSelector">
      ...
      <ul v-if="showOptions">
        <li><a href="#" @click.prevent="changePageSize(item)" :key="item" v-for="item in availableItemsPerPage.filter(i => i !== paginationSize)">{{item}}</a></li>
      </ul>
    </div>
    <div class="pagination">
      ...
      <ul class="pagination__numbers">
        <li
          v-if="totalPages > 1 && !pages.includes(1)">
          <a href="#"
        ...
        <li v-if="pages.length && totalPages > 1 && !pages.includes(1 + 1)">...</li>
        <li
          :key="i"
          v-for="i in pages"
          v-if="totalPages > 1">
    ...
    <div class="pagination__info">
      <strong>Records:</strong> {{(paginationSize * currentPage) -  (paginationSize - 1) }} - {{ paginationSize * currentPage > totalItems ? totalItems : paginationSize * currentPage}} of {{totalItems}} 
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods:{
    ...
    changePageSize(pageSize) {
      ...
      this.$emit('changePage', this.paginationSize === pageSize ? this.currentPage : 1, pageSize);
    },
