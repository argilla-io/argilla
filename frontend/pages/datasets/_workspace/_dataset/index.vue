...
<template>
    ...
    <div :class="[
        ...
        <app-header :dataset="dataset" :breadcrumbs="breadcrumbs" :enableSimilaritySearch="isReferenceRecord" @search-records="searchRecords">
            <task-sidebar v-if="dataset" :dataset="dataset" />
        </app-header>
        <error v-if="$fetchState.error" link="/datasets" :where="datasetName" :error="$fetchState.error"></error>
    ...
    methods: {
        ...
        async activateVectorAndRecorByUrlQueryParams() {
            ...
            if (vectorId) {
                ...
                await VectorModel.update({
                    where: v => v.vector_name === vector_name && v.dataset_id === this.dataset.id.join(".") && v.record_id === record_id,
                    data: {
                        vector_name,
                        is_active: true,
                    },
        ...
        async fetchRecordReferenceAndInsertIntoTheRefRecordModel(recordId) {
            try {
                ...
                console.log("Error on fetchRecordReferenceAndInsertIntoTheRefRecordModel", err);
            }
        },
    },
};
