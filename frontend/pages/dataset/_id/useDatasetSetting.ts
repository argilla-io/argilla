...
import {
	DatasetRepository,
	TYPE_OF_FEEDBACK,
} from "~/v1/infrastructure/DatasetRepository";
import { useDataset } from "~/v1/infrastructure/DatasetStorage";

export const useDatasetSetting = () => {
	const { state: dataset, ...datasetStorage } = useDataset();
	...
	const loadDataset = async (id: string, axios, router) => {
		const datasetRepository = new DatasetRepository(axios, null); //Temporal null;
		const getDatasetUseCase = new GetDatasetByIdUseCase(
			datasetRepository,
			datasetStorage
		);
