...
class ElasticSearchBackendErrorHandler(BaseBackendErrorHandler):
    ...
    """Implements the elastic search backend handler"""
...
class OpensearchBackend(LoggingMixin):

    """Implements the opensearch backend, temporarily copy of the old implementation"""
    ...
    def _get_document_by_id(self, index: str, doc_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.__client__.exists(index=index, id=doc_id):
                return self.__client__.get(index=index, id=doc_id)
        except NotFoundError:  ## maybe opensearchpy.NotFoundError
            return None
        except Exception as ex:
            with self.error_handling(index=index):
                raise ex
    ...
    def error_handling(index: str):
        """Wraps error from the server backend and normalizes them into generic argilla errors"""
        return OpensearchBackendErrorHandler(index=index)
