...
class Argilla:
    ...
    def list_datasets(self) -> List[FeedbackDatasetModel]:
        """Lists all the available `FeedbackTask` datasets in Argilla.

        Returns:
            A list of `FeedbackDatasetModel` objects, containing the dataset
            attributes: id, name, workspace_id, guidelines, status, created_at, and last_updated.
        """
    ...
    def create_dataset(self, name: str, workspace_id: str, guidelines: Optional[str] = None) -> FeedbackDatasetModel:
        """Creates a new `FeedbackTask` dataset in Argilla.

        Args:
            name: The name of the dataset.
            workspace_id: The id of the workspace in which the dataset will be created.
            guidelines: The guidelines of the dataset. Defaults to `None`.

        Returns:
            A `FeedbackDatasetModel` object, containing the dataset attributes: id, name,
            workspace_id, guidelines, status, created_at, and last_updated.
        """
    ...
    def get_dataset(self, id: str) -> FeedbackDatasetModel:
        """Gets a `FeedbackTask` dataset from Argilla.
    ...
    def delete_dataset(self, id: str) -> None:
        """Deletes a `FeedbackTask` dataset from Argilla.

        Args:
            id: The id of the dataset to delete.
        """
        datasets_api_v1.delete_dataset(client=self._client.httpx, id=id)

    def publish_dataset(self, id: str) -> FeedbackDatasetModel:
        """Publishes a `FeedbackTask` dataset in Argilla.

        Args:
            id: The id of the dataset to publish.
    ...
    def get_records(self, id: str, offset: int = 0, limit: int = 50) -> FeedbackRecordsModel:
        """Gets the records of a `FeedbackTask` dataset from Argilla.

        Args:
            id: The id of the dataset to get the records from.
            offset: The offset of the records to get. Defaults to `0`.
            limit: The limit of the records to get. Defaults to `50`.

        Returns:
            A `FeedbackRecordsModel` object, containing the records attributes: items and total,
            where items contains a list of `FeedbackItemModel` objects, with the attributes: id,
            fields, responses, external_id, inserted_at, and updated_at.
        """
    ...
    def add_records(self, id: str, records: List[Dict[str, Any]]) -> None:
        """Adds records to a `FeedbackTask` dataset in Argilla.

        Args:
            id: The id of the dataset to add the records to.
            records: A list of records to add to the dataset, each record being a dictionary
                containing the record attributes: fields, responses, and external_id.
        """
    ...
    def get_fields(self, id: str) -> List[FeedbackFieldModel]:
        """Gets the fields of a `FeedbackTask` dataset from Argilla.

        Args:
            id: The id of the dataset to get the fields from.

        Returns:
            A list of `FeedbackFieldModel` objects, containing the field attributes: id, name,
            title, required, settings, inserted_at, and updated_at.
        """
        return datasets_api_v1.get_fields(client=self._client.httpx, id=id).parsed

    def add_field(self, id: str, field: Dict[str, Any]) -> None:
        """Adds a field to a `FeedbackTask` dataset in Argilla.

        Args:
            id: The id of the dataset to add the field to.
            field: A dictionary containing the field attributes: name, title, required, and settings.
        """
        datasets_api_v1.add_field(client=self._client.httpx, id=id, field=field)

    def get_questions(self, id: str) -> List[FeedbackQuestionModel]:
        """Gets the questions of a `FeedbackTask` dataset from Argilla.

        Args:
            id: The id of the dataset to get the questions from.

        Returns:
            A list of `FeedbackQuestionModel` objects, containing the question attributes: id,
            name, title, type, description, required, settings, inserted_at, and updated_at.
        """
        return datasets_api_v1.get_questions(client=self._client.httpx, id=id).parsed

    def add_question(self, id: str, question: Dict[str, Any]) -> None:
        """Adds a question to a `FeedbackTask` dataset in Argilla.

        Args:
            id: The id of the dataset to add the question to.
            question: A dictionary containing the question attributes: name, title, type,
                description, required, and settings.
        """
        datasets_api_v1.add_question(client=self._client.httpx, id=id, question=question)
