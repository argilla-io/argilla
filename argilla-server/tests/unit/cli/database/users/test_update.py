...
def test_update(
    sync_db: "Session", cli_runner: CliRunner, cli: Typer, new_role_string: str
):
    user = UserSyncFactory.create(username="username", role=UserRole.annotator)

    result = cli_runner.invoke(
        cli, f"database users update username --role {new_role_string}"
    )
...
def test_update_with_invalid_role(cli_runner: CliRunner, cli: Typer):
    ...
    result = cli_runner.invoke(
        cli, f"database users update username --role {bad_role_str}"
    )
...
def test_update_with_missing_username(cli_runner: CliRunner, cli: Typer):
    ...
    result = cli_runner.invoke(
        cli, f"database users update {missing_username} --role owner"
    )

    assert result.exit_code == 0
    assert (
        result.output
        == f"User with username {missing_username!r} does not exists in database. Skipping...\n"
    )


@pytest.mark.parametrize("role_string", ["owner", "admin", "annotator"])
def test_update_with_same_user_role(
    cli_runner: CliRunner, cli: Typer, role_string: str
):
    username = "username"
    UserSyncFactory.create(username=username, role=UserRole(role_string))

    result = cli_runner.invoke(
        cli, f"database users update {username} --role {role_string}"
    )

    assert result.exit_code == 0
    assert (
        result.output
        == f"User {username!r} already has role {role_string!r}. Skipping...\n"
    )
