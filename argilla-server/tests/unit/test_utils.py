...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"keys": ["key1", "key2"], "key3": ["value1", "value2"]},
        ),
        ...
        (
            {"max_keys": 1, "max_values_per_key": 1},
            ["key1:value1"],
        ...
        (
            {"max_values_per_key": 1, "group_keys_without_values": False},
            ["key1", "key2"],
            {"key1": None, "key2": None},
        ),
        (
        ...
        (
            {"group_keys_without_values": True},
            ["key1,key2,key3"],
            {"keys": ["key1", "key2", "key3"]},
        ),
    ],
...
def test_parse_query_param(
    config: Dict[str, Any], params: List[str], expected: Dict[str, Set[str]]
) -> None:
...
def test_parse_query_param_with_base_model() -> None:
    ...
    result = parse_function(
        param_values=["key1", "key2", "key3,key4", "key5:value1,value2,value3"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"max_keys": 1},
            ["key1,key2"],
            "'unit-test' query parameter must contain at most 1 comma-separated keys",
        ),
        (
...
def test_parse_query_param_raises_http_exception(
    config: Dict[str, Any], params: List[str], expected_msg: str
) -> None:
    parse_function = parse_query_param(name="unit-test", **config)
