...
class TestSuiteVectorsSettings:
    ...
    @pytest.mark.parametrize(
        "title", [None, "", "t" * (VECTOR_SETTINGS_CREATE_TITLE_MAX_LENGTH + 1)]
    )
    ...
    async def test_update_vector_settings_non_existent(
        self, async_client: "AsyncClient", owner_auth_header: dict
    ):
        ...
        assert response.json() == {
            "detail": f"VectorSettings with id `{vector_settings_id}` not found"
        }
    ...
    async def test_update_vector_settings_as_annotator(
        self, async_client: "AsyncClient", owner_auth_header: dict
    ):
        vector_settings = await VectorSettingsFactory.create()
        user = await UserFactory.create()
    ...
    @pytest.mark.parametrize("role", [UserRole.owner, UserRole.admin])
    async def test_delete_vector_settings(
        self, async_client: "AsyncClient", role: UserRole
    ):
        vector_settings = await VectorSettingsFactory.create()
        user = await UserFactory.create(
    ...
    async def test_delete_vector_settings_non_existing(
        self, async_client: "AsyncClient", owner_auth_header: dict
    ):
        vector_settings_id = uuid4()
    ...
    async def test_delete_vector_settings_as_annotator(
        self, async_client: "AsyncClient"
    ):
        vector_settings = await VectorSettingsFactory.create()
        annotator = await AnnotatorFactory.create(
            workspaces=[vector_settings.dataset.workspace]
        )
    ...
    async def test_delete_vector_settings_as_admin_from_another_workspace(
        self, async_client: "AsyncClient"
    ):
        vector_settings = await VectorSettingsFactory.create()
    ...
    async def test_delete_vector_settings_without_authentication(
        self, async_client: "AsyncClient"
    ):
        vector_settings = await VectorSettingsFactory.create()
