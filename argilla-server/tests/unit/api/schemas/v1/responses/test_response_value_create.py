...
class TestResponseValueCreate:
    @pytest.mark.parametrize(
        "value", ["hello", "0", "42", "42.0", "42.5", "-1", "True", "False"]
    )
    ...
    def test_with_ranking_value(self):
        ...
        assert ResponseValueCreate(
            value=[{"value": "value-1", "rank": 1}, {"value": "value-2", "rank": 2}]
        ).value == [
            RankingQuestionResponseValueItem(value="value-1", rank=1),
    ...
    def test_with_span_value(self):
        assert ResponseValueCreate(
            value=[{"label": "label-1", "start": 0, "end": 1}]
        ).value == [SpanQuestionResponseValueItem(label="label-1", start=0, end=1)]
        assert ResponseValueCreate(
            value=[
