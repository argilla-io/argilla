...
def es_range_query(
    field_name: str, gte: Optional[float] = None, lte: Optional[float] = None
) -> dict:
    query = {}
    if gte is not None:
        query["gte"] = gte
...
def es_field_for_metadata_property(
    metadata_property: Union[str, MetadataProperty],
) -> str:
...
def es_mapping_for_field(field: Field) -> dict:
    ...
    if field_type == FieldType.text:
        ...
        raise Exception(
            f"Index configuration for field of type {field_type} cannot be generated"
        )
...
def es_mapping_for_metadata_property(metadata_property: MetadataProperty) -> dict:
    ...
    if property_type == MetadataPropertyType.terms:
        ...
        raise Exception(
            f"Index configuration for metadata property of type {property_type} cannot be generated"
        )
...
def _unify_metadata_filters_with_filter(
    metadata_filters: List[MetadataFilter], filter: Optional[Filter]
) -> Filter:
    filters = []
    if filter:
    ...
    for metadata_filter in metadata_filters:
        metadata_scope = MetadataFilterScope(
            metadata_property=metadata_filter.metadata_property.name
        )
        if isinstance(metadata_filter, TermsMetadataFilter):
            new_filter = TermsFilter(
                scope=metadata_scope, values=metadata_filter.values
            )
        elif isinstance(metadata_filter, (IntegerMetadataFilter, FloatMetadataFilter)):
            new_filter = RangeFilter(
                scope=metadata_scope, ge=metadata_filter.ge, le=metadata_filter.le
            )
        else:
            raise ValueError(
                f"Cannot process request for metadata filter {metadata_filter}"
            )
...
def _unify_user_response_status_filter_with_filter(
    ...
    scope = ResponseFilterScope(
        user=user_response_status_filter.user, property="status"
    )
    response_filter = TermsFilter(
        scope=scope,
        values=[status.value for status in user_response_status_filter.statuses],
    )
...
def is_response_status_scope(scope: FilterScope) -> bool:
    return (
        ...
        and scope.property == "status"
        and scope.question is None
    )
...
def _get_response_value_fields_for_question(
    index_mapping: dict, question: str
) -> List[str]:
    ...
    response_fields = []
    for user_id, user_responses in (
        mapping_properties["responses"].get("properties", {}).items()
    ):
        if question in user_responses["properties"]["values"]["properties"]:
            response_fields.append(
                es_field_for_response_value(User(id=UUID(user_id)), question=question)
            )
...
class BaseElasticAndOpenSearchEngine(SearchEngine):
    ...
    async def delete_records(self, dataset: Dataset, records: Iterable[Record]):
        ...
        bulk_actions = [
            {"_op_type": "delete", "_id": record.id, "_index": index_name}
            for record in records
        ]
    ...
    async def update_record_response(self, response: Response):
        ...
        await self._update_document_request(
            index_name, id=record.id, body={"doc": {"responses": es_responses}}
        )
    ...
    async def delete_record_response(self, response: Response):
        ...
        await self._update_document_request(
            index_name,
            id=record.id,
            body={"script": es_script_for_delete_user_response(response.user)},
        )
    ...
    async def delete_record_suggestion(self, suggestion: Suggestion):
        ...
        await self._update_document_request(
            ...
            id=suggestion.record_id,
            body={
                "script": f'ctx._source["suggestions"].remove("{suggestion.question.name}")'
            },
        )
        ...
        bulk_actions = [
            {
                ...
                "_index": index_name,
                "doc": {
                    es_field_for_vector_settings(vector.vector_settings): vector.value
                },
            }
    ...
    async def similarity_search(
        ...
        if user_response_status_filter and user_response_status_filter.statuses:
            filter = _unify_user_response_status_filter_with_filter(
                user_response_status_filter, filter
            )
        # END TODO

        if bool(value) == bool(record):
            raise ValueError(
                "Must provide either vector value or record to compute the similarity search"
            )
    ...
    def build_elasticsearch_filter(
        self, filter: Filter, index_mapping: dict
    ) -> Dict[str, Any]:
        if isinstance(filter, AndFilter):
            filters = [
                self.build_elasticsearch_filter(f, index_mapping)
        ...
        if is_response_status_scope(filter.scope):
            status_filter = UserResponseStatusFilter(
                ...
                statuses=[ResponseStatusFilter(v) for v in filter.values],
            )
    ...
    def _scope_to_elasticsearch_field(scope: FilterScope) -> str:
        if isinstance(scope, MetadataFilterScope):
            ...
            return es_field_for_suggestion_property(
    ...
    @staticmethod
    def _build_response_status_filter(
        status_filter: UserResponseStatusFilter,
    ) -> Dict[str, Any]:
        ...
        if status_filter.has_pending_status:
            ...
            filters.append(
                es_bool_query(must_not=es_exists_field_query(response_field))
            )

        if status_filter.response_statuses:
            # See https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-terms-query.html
            filters.append(
                es_terms_query(response_field, values=status_filter.response_statuses)
            )

        return es_bool_query(should=filters, minimum_should_match=1)

    def _build_response_value_filter_without_user(
        self, filter: Filter, index_mapping: dict
    ) -> dict:
        ...
        """
        question_response_fields = _get_response_value_fields_for_question(
            index_mapping, filter.scope.question
        )
        all_user_filters = [
        ...
        return es_bool_query(
            must=es_exists_field_query(ALL_RESPONSES_STATUSES_FIELD),
            should=all_user_filters,
    ...
    def _map_record_to_es_document(self, record: Record) -> Dict[str, Any]:
        ...
        if record.metadata_:
            document["metadata"] = self._map_record_metadata_to_es(
                record.metadata_, record.dataset.metadata_properties
            )
        if record.responses:
            document["responses"] = self._map_record_responses_to_es(record.responses)
        if record.suggestions:
            document["suggestions"] = self._map_record_suggestions_to_es(
                record.suggestions
            )
        if record.vectors:
    ...
    def _map_record_vectors_to_es(vectors: List[Vector]) -> Dict[str, List[float]]:
        return {
            es_path_for_vector_settings(vector.vector_settings): vector.value
            for vector in vectors
        }
    ...
    async def search(
        ...
        if filter:
            index_mapping = await self.client.indices.get_mapping(index=index)
            bool_query["filter"] = self.build_elasticsearch_filter(
                filter, index_mapping
            )
        ...
        if user_id:
            ...
            es_query = {
                "function_score": {
                    "query": es_query,
                    "functions": [
                        {"random_score": {"seed": str(user_id), "field": "_seq_no"}}
                    ],
                }
            }

        es_sort = self.build_elasticsearch_sort(sort) if sort else None
        response = await self._index_search_request(
            index, query=es_query, size=limit, from_=offset, sort=es_sort
        )
    ...
    async def compute_metrics_for(
        ...
        if metadata_property.type in [
            MetadataPropertyType.float,
            MetadataPropertyType.integer,
        ]:
            return await self._metrics_for_numeric_property(
                index_name, metadata_property
            )
    ...
    async def _metrics_for_numeric_property(
        ...
        metrics_class = (
            IntegerMetadataMetrics
            if metadata_property.type == MetadataPropertyType.integer
            else FloatMetadataMetrics
        )
    ...
    async def _metrics_for_terms_property(
        ...
        total_terms = await self.__value_count_aggregation(
            index_name, field_name=field_name, query=query
        )
        if total_terms == 0:
            return TermsMetadataMetrics(total=total_terms)

        terms_buckets = await self.__terms_aggregation(
            index_name, field_name=field_name, query=query, size=total_terms
        )
        terms_values = [
            TermsMetadataMetrics.TermCount(
    ...
    def _configure_index_mappings(self, dataset: Dataset) -> dict:
        return {
            ...
            "dynamic": "strict",
            "dynamic_templates": self._dynamic_templates_for_question_responses(
                dataset.questions
            ),
            "properties": {
    ...
    async def _process_search_response(
        ...
        items = [
            SearchResponseItem(record_id=UUID(hit["_id"]), score=hit["_score"])
            for hit in hits
        ]
    ...
    def _build_text_query(
        dataset: Dataset, text: Optional[Union[TextQuery, str]] = None
    ) -> dict:
        if text is None:
            return {"match_all": {}}
        ...
        if not text.field:
            ...
            return {
                "match": {
                    es_field_for_record_field(text.field): {
                        "query": text.q,
                        "operator": "and",
                    }
                }
    ...
    def _mapping_for_metadata_properties(
        self, metadata_properties: List[MetadataProperty]
    ) -> dict:
        mappings = {
    ...
    def _dynamic_templates_for_question_responses(
        ...
        return [
            {
                "status_responses": {
                    ...
                    "mapping": {
                        "type": "keyword",
                        "copy_to": ALL_RESPONSES_STATUSES_FIELD,
                    },
                }
            },
            *[
    ...
    def _mapping_for_vectors_settings(
        self, vectors_settings: List[VectorSettings]
    ) -> dict:
        mappings = {}
    ...
    async def __terms_aggregation(
        self, index_name: str, field_name: str, query: dict, size: int
    ) -> List[dict]:
        aggregation_name = "terms_agg"

        terms_agg = {
            aggregation_name: {
                "terms": {"field": field_name, "size": min(size, self.max_terms_size)}
            }
        }

        response = await self._index_search_request(
            index_name, query=query, aggregations=terms_agg, size=0
        )
        return response["aggregations"][aggregation_name]["buckets"]

    async def __value_count_aggregation(
        self, index_name: str, field_name: str, query: dict
    ) -> int:
        ...
        response = await self._index_search_request(
            index_name, query=query, aggregations=value_count_agg, size=0
        )
        return response["aggregations"][aggregation_name]["value"]

    async def __stats_aggregation(
        self, index_name: str, field_name: str, query: dict
    ) -> dict:
        ...
        response = await self._index_search_request(
            index_name, query=query, aggregations=stats_agg, size=0
        )
