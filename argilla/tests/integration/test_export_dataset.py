...
@pytest.fixture
def token():
    return os.getenv("HF_TOKEN_ARGILLA_INTERNAL_TESTING") or os.getenv("HF_TOKEN")


@pytest.mark.parametrize("with_records", [True, False])
class TestDiskImportExportMixin:
    def test_export_dataset_to_disk(self, dataset: rg.Dataset, mock_data: List[dict[str, Any]], with_records: bool):
        dataset.records.log(records=mock_data)

        with TemporaryDirectory() as temp_dir:
            output_dir = dataset.to_disk(path=temp_dir, with_records=with_records)
    ...
    def test_import_dataset_from_disk(
        self, dataset: rg.Dataset, client, mock_data: List[dict[str, Any]], with_records: bool
    ):
        dataset.records.log(records=mock_data)

        with TemporaryDirectory() as temp_dir:
            output_dir = dataset.to_disk(path=temp_dir, with_records=with_records)
            new_dataset = rg.Dataset.from_disk(output_dir, client=client, with_records=with_records)
...
class TestHubImportExportMixin:
    def test_export_dataset_to_hub(
        self, token: str, dataset: rg.Dataset, mock_data: List[dict[str, Any]], with_records: bool
    ):
        repo_id = f"argilla-internal-testing/test_export_dataset_to_hub_with_records_{with_records}"
        dataset.records.log(records=mock_data)
        dataset.to_hub(repo_id=repo_id, with_records=with_records, token=token)

    def test_import_dataset_from_hub(
        self, token: str, dataset: rg.Dataset, client, mock_data: List[dict[str, Any]], with_records: bool
    ):
        repo_id = f"argilla-internal-testing/test_import_dataset_from_hub_with_records_{with_records}"
        dataset.records.log(records=mock_data)

        dataset.to_hub(repo_id=repo_id, with_records=with_records, token=token)
        new_dataset = rg.Dataset.from_hub(repo_id=repo_id, with_records=with_records, token=token)
